...
export function Calendar() {
    ...
    const changeMonth = (e: any) => {
        ...
        // setAllCalendarDay(getAllCalendarDay(date));
    }
    ...
    return (
        <div>
            <div onClick={toggleCalendar} className="result text-white text-xs border-solid border-2 border-gray-400 rounded-lg">{finalSelectDay && moment(finalSelectDay.date).format("DD/MM/YYYY")}</div>
            <div className={isShowCalendar ? "calendar visible" : "invisible hidden"}>
                <h1 className='ml-6 text-white text-xs'>Text</h1>
                <h1 className="ml-6 w-screen text-white text-left selectDay">{moment(selectDay.date).format('MMM, YYYY')}</h1>
                <div className="flex items-center justify-between selectCalendarday">
                    ...
                    <h1 onClick={toggleMonthAYear} className="focus:outline-none  text-base dark:text-gray-100 text-gray-800 calendarday">{isMonth ? moment(calendarDay.date).format('MMMM YYYY') : selectedYear}</h1>
                ...
                {<div className={isMonth ? "visible" : "invisible hidden"}>
                    <div className='flex flex-row justify-between weekTitleContainer'>{weeklist.map((element, index) =>
